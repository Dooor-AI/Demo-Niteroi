              ...
              <CardContent className="space-y-3">
                {competencies.map((competency) => (
                  <div key={competency.id} className="flex items-center justify-between">
                    ...
                    <Badge
                      variant={competency.active ? "default" : "secondary"}
                      className={`text-xs ${competency.active ? "bg-green-100 text-green-800 font-medium" : "font-normal"}`}
                    >
                      {competency.active ? "Ativa" : "Inativa"}
                    </Badge>
                  </div>
                ))}
              </CardContent>
